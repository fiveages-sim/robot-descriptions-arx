...
升降底盘 + 双臂（X5/R5）+ 头部。控制模式：**分体 / 全身**。

| 模式 | Launch | 规划 | 升降 / 头 |
|------|--------|------|------|
| 全身 | `full_body.launch.py` | 本包 `task.info`（`ocs2_wheel_humanoid`） | `ocs2_wbc_controller`（lift + 双臂 + 头） |
| 分体 | `split_body.launch.py` | **`arx_acone`** `task.info`（根 `arm_base`） | `body_joint_controller` + `head_joint_controller` |
